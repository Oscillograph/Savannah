			...
			case ProforientatorTasks::EditSkillGroup:
				{
					if (m_EditGroup != nullptr)
					{
						...
						for (size_t i = 0; i < m_SkillGroupSelected->children.size(); i++)
						{
...
//				ImGui::ShowStackToolWindow();
			
			{
				if (ImGui::BeginTable("Middle Table", (m_SkillsRegistry->GetGroupsNames().size() + 2), ImGuiTableFlags_RowBg | ImGuiTableFlags_ScrollX, {TEXT_BASE_WIDTH * 140, TEXT_BASE_HEIGHT * 15}))
				{
					...
					for (size_t i = 0; i <= m_SkillsRegistry->GetGroupsNames().size(); i++)
					{
						sprintf((char*)columnName.c_str(), "##Group%d", (int)i);
...
//						SAVANNAH_CONSOLE_LOG("Groupless skills table shown.");
	...
	void Proforientator::ProcessSkillRequirements()
	{
		if (m_SkillSelected != nullptr)
		{
			...
			for (size_t i = 0; i < m_SkillSelected->GetRequirementsArray().size(); i++)
			{
...
Ползунком отмечается уровень мастерства с оценкой от 0 до 10.)";
	...
	void Proforientator::ShowSkillRequirementsTable()
	{
		if (ImGui::BeginTable("##Требования", 3, ImGuiTableFlags_SizingFixedFit | ImGuiTableFlags_ScrollY))
		{
			...
			for (size_t i = 0; i < m_SkillRequirementsMet.size(); i++)
			{
...
//				uint32_t& reqLevel = m_SkillRequirementsMet[i]->level;
			...
			for (size_t i = 0; i < m_SkillRequirementsNotMet.size(); i++)
			{
	...
	void Proforientator::ShowEditSkillTable()
	{
		...
		if (ImGui::BeginTable((columnsID.c_str()), 3, ImGuiTableFlags_SizingFixedFit))
		{
			...
			ImGui::TableSetColumnIndex(1);
			{
				...
				{
					m_EditSkill->group = ((size_t)comboSelected == m_SkillsRegistry->GetGroupsNames().size()) ? "Groupless" : m_SkillsRegistry->GetGroupsNames()[comboSelected];
				}
			}
...
//						SAVANNAH_CONSOLE_LOG(m_SkillSelected->GetRequirementsArray().size());
					...
					ImGui::TableSetColumnIndex(2); // all skills
					{
						...
						{
							bool selected = false;
							for (size_t i = 0; i < m_SkillsRegistry->GetAllSkills().size(); i++)
							{
								ImGui::TableNextRow();
								ImGui::TableSetColumnIndex(0);
